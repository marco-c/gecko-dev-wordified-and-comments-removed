...
proto
.
has_rtx_original_sequence_number
(
)
)
{
...
>
>
sequence_number_values
=
DecodeDeltas
(
proto
.
sequence_number_deltas
(
)
proto
.
...
>
>
rtp_timestamp_values
=
DecodeDeltas
(
proto
.
rtp_timestamp_deltas
(
)
proto
.
...
proto
.
transport_sequence_number_deltas
(
)
base_transport_sequence_number
number_of_deltas
...
proto
.
transmission_time_offset_deltas
(
)
unsigned_base_transmission_time_offset
number_of_deltas
...
proto
.
rtx_original_sequence_number_deltas
(
)
base_rtx_osn
number_of_deltas
...
proto_header_extensions
.
has_transmission_time_offset_id
(
)
)
{
...
proto_header_extensions
.
has_transport_sequence_number_id
(
)
)
{
...
stream
.
incoming_rtp_packets_size
(
)
+
stream
.
outgoing_rtp_packets_size
(
)
+
stream
...
stream
.
loss_based_bwe_updates_size
(
)
+
stream
.
delay_based_bwe_updates_size
(
)
+
stream
.
scream_bwe_updates_size
(
)
+
stream
.
dtls_transport_state_events_size
(
)
+
stream
.
dtls_writable_states_size
(
)
+
stream
.
audio_network_adaptations_size
(
)
+
stream
...
stream
.
remote_estimates_size
(
)
+
stream
.
ice_candidate_configs_size
(
)
+
stream
.
ice_candidate_events_size
(
)
+
stream
.
audio_recv_stream_configs_size
(
)
+
stream
.
audio_send_stream_configs_size
(
)
+
stream
.
video_recv_stream_configs_size
(
)
+
stream
.
video_send_stream_configs_size
(
)
+
stream
.
generic_packets_sent_size
(
)
+
stream
.
generic_packets_received_size
(
)
+
stream
.
generic_acks_received_size
(
)
+
stream
.
frame_decoded_events_size
(
)
+
stream
.
neteq_set_minimum_delay_size
(
)
1u
)
...
proto
.
link_capacity_lower_kbps_deltas
(
)
base_link_capacity_lower_kbps
number_of_deltas
...
>
>
minimum_delay_ms_values
=
DecodeDeltas
(
proto
.
minimum_delay_ms_deltas
(
)
ToUnsigned
(
...
>
>
ref_window_bytes_values
=
DecodeDeltas
(
proto
.
ref_window_bytes_deltas
(
)
proto
.
...
>
>
data_in_flight_bytes_values
=
DecodeDeltas
(
proto
.
data_in_flight_bytes_deltas
(
)
proto
.
...
>
>
target_rate_kbps_values
=
DecodeDeltas
(
proto
.
target_rate_kbps_deltas
(
)
proto
.
...
>
>
smoothed_rtt_ms_values
=
DecodeDeltas
(
proto
.
smoothed_rtt_ms_deltas
(
)
proto
.
...
>
>
avg_queue_delay_ms_values
=
DecodeDeltas
(
proto
.
avg_queue_delay_ms_deltas
(
)
proto
.
...
>
>
l4s_marked_permille_values
=
DecodeDeltas
(
proto
.
l4s_marked_permille_deltas
(
)
proto
.
...
>
>
uplink_packet_loss_fraction_values
=
DecodeDeltas
(
proto
.
uplink_packet_loss_fraction_deltas
(
)
uplink_packet_loss_fraction
number_of_deltas
