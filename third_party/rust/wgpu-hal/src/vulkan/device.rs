...
self
.
create_image_without_memory_with_tiling
(
desc
vk
:
...
let
mut
vk_rasterization_conservative_state
=
vk
:
:
...
private_caps
.
ray_tracing_pipeline_group_data_size
)
as
usize
)
...
:
:
map_acceleration_structure_flags
(
desc
.
flags
...
:
:
map_acceleration_structure_format
(
desc
.
format
