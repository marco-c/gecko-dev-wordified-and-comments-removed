...
def
test_post_with_multipart__iter__
(
self
)
:
...
def
test_percent_encode_invalid_target_chars
(
self
)
:
...
def
test_read_chunked_short_circuit
(
self
)
:
...
def
test_read_chunked_on_closed_response
(
self
)
:
...
def
test_preserves_path_dot_segments
(
self
)
:
...
def
test_retries_wrong_method_list
(
self
)
:
...
1
class
TestFileBodiesOnRetryOrRedirect
(
HTTPDummyServerTestCase
)
:
