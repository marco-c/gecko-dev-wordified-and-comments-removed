...
{
return
LoadStorageAccessReloadChannel
(
)
;
}
...
:
:
network_lna_block_insecure_contexts
(
)
&
&
...
:
:
network_lna_allow_top_level_navigation
(
)
)
{
...
:
:
network_mimesniff_sniff_misconfigured_types
(
)
)
{
...
rv
=
DoApplyContentConversionsInternal
(
mListener
getter_AddRefs
(
...
{
StoreCacheEntryIsNoVarySearchMatch
(
false
)
;
...
(
!
LoadCacheEntryIsNoVarySearchMatch
(
)
|
|
...
:
:
network_prefetch_next_force_valid_for
(
)
;
if
...
:
:
network_cache_suspended_writer_delay_ms
(
)
;
if
...
:
:
network_http_happy_eyeballs_upgrade_enabled
(
)
)
{
...
-
>
StoreStorageAccessReloadChannel
(
true
)
;
...
:
:
LocalNetworkAccessConnectionExtra
extra
=
{
.
...
:
:
network_cache_entry_wait_timeout_ms
(
)
;
if
