...
:
:
network_dns_resolver_thread_extra_idle_time_seconds
(
)
;
uint32_t
...
:
:
network_dnsCacheExpiration
(
)
StaticPrefs
:
:
network_dnsCacheExpirationGracePeriod
(
)
)
;
...
:
:
network_dns_always_ai_canonname
(
)
)
{
...
:
:
network_dns_port_prefixed_qname_https_rr
(
)
&
&
...
:
:
network_dns_refresh_negative_addr_on_use
(
)
;
if
...
:
:
network_trr_strict_native_fallback_allow_timeouts
(
)
)
{
...
:
:
network_dns_negative_ttl_for_type_record
(
)
0
)
