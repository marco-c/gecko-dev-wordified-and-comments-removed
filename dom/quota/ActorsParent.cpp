...
#
define
SHUTDOWN_CRASH_BROWSER_TIMEOUT_MS
45000
static_assert
(
SHUTDOWN_CRASH_BROWSER_TIMEOUT_MS
...
nsresult
>
ReadFullOriginMetadataFromMetadataV2File
(
nsIFile
*
aDirectory
...
nsresult
>
UpdateFullOriginMetadataPrincipalProperties
(
QuotaManager
&
aQuotaManager
...
:
:
quotamanager_initialize_repository
:
:
...
:
:
dom_quotaManager_originInitialization_pauseOnIOThreadMs
(
)
)
;
...
:
:
dom_quotaManager_storageInitialization_pauseOnIOThreadMs
(
)
;
if
...
:
:
dom_quotaManager_originWriteQuiescentThresholdMs
(
)
)
;
const
auto
writeQuiescentThresholdDuration
=
TimeDuration
:
:
...
const
uint32_t
maxOriginsToSaveInOneBatch
=
StaticPrefs
:
:
dom_quotaManager_maxOriginsToSaveInOneBatch
(
)
;
MutexAutoTryLock
...
UniversalDirectoryLock
>
firstAccessDirectoryLock
=
CreateSaveOriginAccessTimeLock
(
*
this
aClientMetadata
...
=
CreatePersistentStorageInitializedOp
(
WrapMovingNotNullUnchecked
(
this
...
:
:
quotamanager_initialize_temporarystorage
:
:
total_time_excluding_suspend
.
...
:
:
dom_quotaManager_originMetadataFlushCheckIntervalMs
(
)
)
;
