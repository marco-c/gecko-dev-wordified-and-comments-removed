...
:
:
GetPrefName_gfx_webrender_debug_slow_cpu_frame_threshold
(
)
10
.
...
:
:
SetWebRenderSlowCpuFrameThreshold
(
threshold
)
;
...
:
:
SetShouldWarmUpWebRenderProgramBinaries
(
warmUp
)
;
...
:
:
SetUseAImageReaderVideoGpuProcessAndroid
(
StaticPrefs
:
:
gfx_video_aimage_reader_gpu_process_android_AtStartup
(
)
)
;
...
:
:
gfx_webrender_dcomp_video_hw_overlay_win_AtStartup
(
)
)
{
...
"
"
FEATURE_FAILURE_DISABLED_BY_PREF
"
_ns
)
;
...
:
:
gfx_webrender_dcomp_video_sw_overlay_win_AtStartup
(
)
)
{
...
:
:
SetWebRenderLayerCompositorDCompTexture
(
true
)
;
...
:
:
gfx_webrender_dcomp_texture_overlay_win_AtStartup
(
)
&
&
...
:
:
gfx_webrender_overlay_hdr_AtStartup
(
)
)
{
...
:
:
SetWebRenderOverlayVpSuperResolution
(
true
)
;
...
:
:
SetCanUseVulkanHardwareVideoDecoding
(
featureVulkanDec
.
IsEnabled
...
"
"
FEATURE_HARDWARE_VIDEO_DECODING_PREF_1_DISABLED
"
_ns
)
;
...
"
"
FEATURE_HARDWARE_VIDEO_DECODING_PREF_2_DISABLED
"
_ns
)
;
...
"
"
FEATURE_FAILURE_SANITY_TEST_FAILED
"
_ns
)
;
...
"
"
FEATURE_FAILURE_OSX_XPCSHELL_SIGNING
"
_ns
)
;
...
"
"
FEATURE_HARDWARE_VIDEO_ENCODING_PREF_1_DISABLED
"
_ns
)
;
...
"
"
FEATURE_HARDWARE_VIDEO_ENCODING_PREF_2_DISABLED
"
_ns
)
;
...
#
define
CODEC_HW_FEATURE_SETUP_PLATFORM
(
name
type
encoder
...
:
:
webgl_ignore_blocklist_AtStartup
(
)
)
{
...
:
:
SetUseAHardwareBufferSharedSurfaceWebglOop
(
StaticPrefs
:
:
webgl_out_of_process_enable_ahardwarebuffer_AtStartup
(
)
)
;
...
:
:
webgl_allow_in_parent_AtStartup
(
)
)
{
...
"
"
FEATURE_FAILURE_THREAD_UNSAFE_GL
"
_ns
)
;
...
:
:
SetWebglOopAsyncPresentForceSync
(
webglOopAsyncPresentForceSync
)
;
...
:
:
widget_dmabuf_export_force_enabled_AtStartup
(
)
)
{
...
:
:
widget_dmabuf_webgl_enabled_AtStartup
(
)
)
{
...
:
:
dom_webgpu_allow_in_parent_AtStartup
(
)
)
{
...
:
:
SetAllowWebGPUPresentWithoutReadback
(
true
)
;
...
:
:
GetPrefName_dom_webgpu_external_texture_enabled
(
)
true
StaticPrefs
:
:
GetPrefDefault_dom_webgpu_external_texture_enabled
(
)
)
;
...
:
:
FEATURE_WEBGPU_EXTERNAL_TEXTURE
&
message
failureId
)
...
"
"
WEBGPU_EXTERNAL_TEXTURE_UNSUPPORTED_OS
"
_ns
)
;
...
:
:
GetPrefName_gfx_canvas_accelerated
(
)
true
StaticPrefs
:
:
GetPrefDefault_gfx_canvas_accelerated
(
)
)
;
...
"
"
FEATURE_FAILURE_DISABLED_BY_GPU_PROCESS_DISABLED
"
_ns
)
;
...
"
"
FEATURE_FAILURE_DISABLED_BY_SOFTWARE_WEBRENDER
"
_ns
)
;
...
:
:
GetPrefName_gfx_canvas_accelerated_force_enabled
(
)
)
)
...
"
"
FEATURE_FAILURE_DISABLED_BY_FALLBACK_GPU_PROCESS_DISABLED
"
_ns
)
;
...
"
"
FEATURE_ACCELERATED_CANVAS2D_RUNTIME_ERROR
"
_ns
)
;
